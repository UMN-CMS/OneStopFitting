...
def loadBackground(
    pipeline: str, category: str, background_base: Path, year: str, resources_dir: Path
):
    bkg_path = (
        background_base
        / year
...
def scanParametersForGroup(
    ...
    bkg_data: BinnedData,
    rebin_factor: int,
    core_thresholds: list,
    dilation_margins: list,
    smooth_sigma: float,
    ...
    for mstop, mchi, sig_path in signal_infos:
        ...
        if signal_smooth_sigma > 0.0:
            ...
            s_arr = gaussian_filter(
                s_arr.reshape(shape), sigma=signal_smooth_sigma
            ).ravel()
        ...
        for ct in core_thresholds:
            for dm in dilation_margins:
                ...
                diagnostic_rows.append(
                    f"{pipeline},{category},{mstop},{mchi},{ct},{dm},{cap_frac},{window_size},{total_significance}"
                )
...
def selectBestParameters(
    ...
    if best_params:
        logger.info(
            f"Selected primary params: ct={best_params['core_threshold_fraction']}, dm={best_params['dilation_margin']} (p5_capture={best_params['p5_capture']:.3f}, size={best_params['mean_window_size_bins']:.1f})"
        )

    if best_params is None:
        logger.warning(
            f"Still no parameters met target for {pipeline} {category}. Picking best median."
        )
        best_median = -1.0
        for (ct, dm), results in group_results.items():
            if not results:
        ...
        if best_params:
            logger.info(
                f"Selected fallback params: ct={best_params['core_threshold_fraction']}, dm={best_params['dilation_margin']} (median_capture={best_params['median_capture']:.3f}, size={best_params['mean_window_size_bins']:.1f})"
            )

    return best_params


def plotDiagnosticHeatmaps(
    pipeline, category, year, group_results, best_params, output_dir
):
    ...
    all_meta = [{"era": {"lumi": 59.8, "energy": 13, "name": year}}]

    im1 = axes[0].imshow(
        median_grid, origin="lower", aspect="auto", cmap="viridis", vmin=0.8, vmax=1.0
    )
    ...
    addCMSBits(
        axes[0],
        all_meta,
        extra_text=f"{pipeline} {category}\nMedian Capture Fraction",
        cms_text_pos=1,
    )
    ...
    im2 = axes[1].imshow(
        min_grid, origin="lower", aspect="auto", cmap="viridis", vmin=0.8, vmax=1.0
    )
...
def plotSignalPointDetails(
    ...
    from fitting.core.data import BinnedData
    from fitting.data.windowing import fitCoreDilatedWindow
    from fitting.diagnostics.plot_utils import (
        plotBinnedData,
        plotBlinding2D,
        addCMSBits,
    )
    ...
    if signal_smooth_sigma > 0.0:
        ...
        shape = tuple(len(e) - 1 for e in sig_data.edges)
        s_arr = gaussian_filter(
            s_raw_arr.reshape(shape), sigma=signal_smooth_sigma
        ).ravel()
    else:
        s_arr = s_raw_arr
    ...
    addCMSBits(
        ax_bg, all_meta, extra_text=f"{pipeline} {category}\nBackground", cms_text_pos=1
    )
    ...
    addCMSBits(
        ax_raw_sig,
        all_meta,
        extra_text=f"{pipeline} {category} ({mstop}, {mchi})\nRaw Signal",
        cms_text_pos=1,
    )
    ...
    addCMSBits(
        ax_smooth_sig,
        all_meta,
        extra_text=f"Smoothed Signal ($\\sigma$={signal_smooth_sigma})",
        cms_text_pos=1,
    )
    ...
    addCMSBits(
        ax_sigmap,
        all_meta,
        extra_text=f"Significance Map\nTotal Sig: {total_sig:.2f}",
        cms_text_pos=1,
    )

    plotBinnedData(ax_windows, sig_binned, cmap="plasma")
    addCMSBits(
        ax_windows, all_meta, extra_text="Windows & Capture Fractions", cms_text_pos=1
    )
    ...
    for idx, (ct, dm) in enumerate(all_params_to_plot):
        try:
            ...
            if (
                ct == best_params["core_threshold_fraction"]
                and dm == best_params["dilation_margin"]
            ):
                label += " [BEST]"
                lw = 3
            else:
            ...
            plotBlinding2D(
                ax_windows, sig_data.edges, sig_data.X, mask, color=c, linewidth=lw
            )
    ...
    fig.savefig(
        plot_dir / f"{pipeline}_{category}_{mstop}_{mchi}_details.pdf",
        bbox_inches="tight",
    )
    plt.close(fig)
...
def generatePlots(
    ...
    plotDiagnosticHeatmaps(
        pipeline, category, year, group_results, best_params, output_dir
    )

    best_results = group_results.get(
        (best_params["core_threshold_fraction"], best_params["dilation_margin"]), []
    )
    if not best_results:
        return
...
@click.option(
    ...
    default="export_complete_with_large",
)
@click.option(
    "--background-base",
    type=click.Path(exists=True, path_type=Path),
    default="smoothed_complete",
)
...
@click.option(
    "--output", type=click.Path(path_type=Path), default="results/window_study"
)
@click.option(
    "--resources-dir", type=click.Path(exists=True, path_type=Path), default="resources"
)
@click.option(
    "--smooth-sigma",
    type=float,
    default=1.5,
    help="Smoothing applied to significance map before thresholding.",
)
@click.option(
    "--signal-smooth-sigma",
    type=float,
...
def main(
    ...
    for (pipeline, category), signal_infos in signals_by_group.items():
        logger.info(
            f"Processing {pipeline} {category} with {len(signal_infos)} signals"
        )

        bkg_data, rebin_factor = loadBackground(
            pipeline, category, background_base, year, resources_dir
        )
        if bkg_data is None:
            continue
    ...
    with open(output / "detailed_results.csv", "w") as f:
        f.write(
            "pipeline,category,mstop,mchi,core_threshold,dilation_margin,capture_fraction,window_size,total_significance\n"
        )
